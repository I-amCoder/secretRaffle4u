...
@section('content')
    @php
        ...
        // echo "<pre>";print_r($paytablerecords);echo "</pre>";
        ...
        foreach ($paytablerecords as $key => $val) {
            ...
            if ($val->amount > 0) {
                ...
                if ($val->draw == 0) {
                    $winnerValue = intval($val->amount);
                    $winnerValueIndex = $val->id;
                    $winnerValueArrayIndex = $key;
                }
            ...
            if ($val->draw > 0 && $play == 'yes') {
                $newDraw = $val->draw - 1;
                DB::table('paytables')
                    ->where('id', $val->id)
                    ->update(['draw' => $newDraw]);
            } elseif ($val->draw == 0 && $play == 'yes') {
                DB::table('paytables')
                    ->where('id', $val->id)
                    ->update(['draw' => $val->reset_count]);
            }
        }
        if ($winnerValue > 0) {
            $winner_random = rand(0, 1);
            if ($winner_random == 1) {
                $winner_draw_array = [2, 4, 6, 8];
            } else {
                $winner_draw_array = [3, 5, 7, 9];
            }
            if (in_array($winnerValueArrayIndex, $winner_draw_array, true)) {
                $find_winner_index = array_search($winnerValueArrayIndex, $winner_draw_array);
                $winner_draw_array[$find_winner_index] = $winner_draw_array[$find_winner_index] - 1;
            }
            $zero_val = $winnerValueArrayIndex;
            $draw_values[] = $boxData[$winnerValueArrayIndex];
            $draw_values[] = $boxData[$winnerValueArrayIndex];
            $draw_values[] = $boxData[$winnerValueArrayIndex];
            $first_val = $winner_draw_array[0];
            $draw_values[] = $boxData[$first_val];
            $draw_values[] = $boxData[$first_val];
            $second_val = $winner_draw_array[1];
            $draw_values[] = $boxData[$second_val];
            $draw_values[] = $boxData[$second_val];
            $third_val = $winner_draw_array[2];
            $draw_values[] = $boxData[$third_val];
            $forth_val = $winner_draw_array[3];
            $draw_values[] = $boxData[$forth_val];
            shuffle($draw_values);
        } else {
            for ($i = 0; $i < 9; $i++) {
                ...
                if (isset($array_values_count[$box_number])) {
                    if ($box_number == -1 && $array_values_count[-1] < 3) {
                        $draw_values[] = $boxData[$random_number];
                    } elseif ($array_values_count[$box_number] < 2 && $box_number > 0) {
                        $draw_values[] = $boxData[$random_number];
                    } else {
                        $draw_values[] = -1;
                    }
            ...
            // if(count($draw_values) < 9){
            // $remaining_boxes = 9-count($draw_values);
            // }
            $array_values_count = array_count_values($draw_values);
            if (isset($array_values_count[-1]) && $array_values_count[-1] > 2) {
                $winnerValue = -1;
            }
        }
        // $array_values_count = array_count_values($draw_values);
        // echo "<pre>";print_r($array_values_count);echo "</pre>";
        // echo "<pre>";print_r($draw_values);echo "</pre>";
        // echo "<pre>";print_r($boxData);echo "</pre>";
        // echo "<pre>";print_r($drawData);echo "</pre>";
        // echo "<pre>";print_r($winnerValue);echo "</pre>";
        // echo "<pre>";print_r(count($draw_values));echo "</pre>";
        ...
        if ($winnerValue > 0) {
            ...
            $remainingPrizeData = DB::table('paytables')
                ->where('amount', $winnerValue)
                ->first();
            ...
            if ($remainingPrizeData->prize_remaining > 0) {
                ...
                DB::table('paytables')
                    ->where('amount', $winnerValue)
                    ->update(['prize_remaining' => $remainingPrize]);
            }
        }
        /**/
        if ($play == 'yes') {
    ...
    <style>
        ...
        .scratch-game-start-description-box p,
        .scratch-game-start-description-box label {
            color: #333;
        }
                    ...
                    <table class="table table-bordered table-hover table-striped">
                        ...
                        @foreach ($paytablerecords as $key => $val)
                            <tr>
                                ...
                                <td>{{ $val->amount > 0 ? Session::get('currency_symbol') . '' . number_format($helpers->convert_to_currency(Session::get('currency'), $val->amount)) : 'Free Play' }}
                                </td>
                            </tr>
                        @endforeach
                    </table>
                ...
                <div class="col-8 pay-tab-col mt-5">
                    @if (count($tickets) == 0)
                        <div class="scratch-game-start-description-box">
                            <p>Scratch and win exciting prize money. This scratch card game features the top prize of
                                ฿{{ $heightest_value }}. Match 3 numbers to win one of the prizes listed in the
                                paytable.</p>
                            <div class="form-group">
                                <label>Purchase the number of games you want to play:</label>
                                <div class="clearfix"></div>
                                <button type="button" class="btn_choose_sent bg_btn_chose_1">
                                    <input type="radio" name="name" checked class="scratch-game-ticket" value="1"
                                        onclick="set_ticket_value()" /> 1 Game
                                </button>
                                <button type="button" class="btn_choose_sent bg_btn_chose_2">
                                    <input type="radio" name="name" class="scratch-game-ticket" value="5"
                                        onclick="set_ticket_value()" /> 5 Games
                                </button>
                                <button type="button" class="btn_choose_sent bg_btn_chose_3">
                                    <input type="radio" name="name" class="scratch-game-ticket" value="10"
                                        onclick="set_ticket_value()" /> 10 Games
                                </button>
                                <input type="hidden" id="scratch-game-ticket-unit-price" value="{{ $data->unit_price }}" />
                                <input type="hidden" id="scratch-game-ticket-unit-price-currency"
                                    value="{{ number_format($helpers->convert_to_currency(Session::get('currency'), $data->unit_price), 2) }}" />
                                <input type="hidden" id="scratch-game-ticket-unit-price-usd"
                                    value="{{ number_format($helpers->convert_to_currency('USD', $data->unit_price), 2) }}" />
                            </div>
                            <p><b>Total:</b> ฿<span
                                    class="total-amount-of-tickets">{{ number_format($data->unit_price * 1, 2) }}</span>
                            </p>
                    ...
                    <div class="playbox" style="background:none;">
                        @if (count($tickets) == 0)
                            <div class="playbox-overlay-wrapper">
                                <img src="{{ asset('assets/images/game/' . $data->photo) }}"
                                ...
                                @foreach ($paytablerecords as $index => $val)
                                    @if (isset($val->photo[0]) && $index <= count($paytablerecords) - 2)
                                        <div class="playbox-overlay-wrapper custom-playbox-image">
                                            <img src="{{ asset('assets/images/game/' . $val->photo) }}"
                                                style="width:100%;" />
                                            {{--  <div class="playbox-overlay-div">Scratch and win the jackpot worth ฿{{ $heightest_value }}</div>  --}}
                                        </div>
                                    @endif
                        ...
                        @else
                            @foreach ($draw_values as $key => $val)
                                @if ($val < 0)
                                    <div id="playbox-{{ $key }}" class="scratchpad">
                                        <div class="playbox-amount">{{ 'Free Play' }}</div>
                                    </div>
                                @else
                                    {{-- <div id="playbox-{{ $key }}" class="scratchpad"><div class="playbox-amount">${{ number_format($val)}}</div></div> --}}
                                    <div id="playbox-{{ $key }}" class="scratchpad">
                                        <div class="playbox-amount">
                                            {{ number_format($helpers->convert_to_currency(Session::get('currency'), $val)) }}
                                        </div>
                                    </div>
                                @endif
                            @endforeach
                    ...
                    <div class="play-btn">
                        @if (count($tickets) > 0 && $play == 'no')
                            <a class="btn btn-sm btn--base btn--custom me-5 px-4" id="something"
                                href="{{ route('scratch_cards_game', ['id' => $data->id]) }}?play=on">Play Game</a>
                        @elseif(count($tickets) == 1 && $play == 'yes')
                            <a class="btn btn-sm btn--base btn--custom me-5 px-4 pull-left" id="clear-all"
                                href="javascript:;">Clear All</a>
                            <a class="btn btn-sm btn--base btn--custom me-5 px-4 pull-right" id="something"
                                href="{{ route('scratch_cards_game', ['id' => $data->id]) }}">Better Luck Next Time</a>
                        @elseif(count($tickets) > 1 && $play == 'yes')
                            <a class="btn btn-sm btn--base btn--custom me-5 px-4 pull-left" id="clear-all"
                                href="javascript:;">Clear All</a>
                            <a class="btn btn-sm btn--base btn--custom me-5 px-4 pull-right" id="something"
                                href="{{ route('scratch_cards_game', ['id' => $data->id]) }}?play=on">Play Again
                                ({{ count($tickets) - 1 }})</a>
                        @endif
                    </div>
                </div>
        ...
        function if_playbox_cleared() {
            ...
            if (playbox_cleared && !swalFired) {
                ...
                if (WinnerValue < 0) {
                    ...
                    WinnerValue1 =
                        '{{ number_format($helpers->convert_to_currency(Session::get('currency'), $winnerValue), 2) }}';
                    swalFired = true;
                    Swal.fire({
