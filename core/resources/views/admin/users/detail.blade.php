...
@section('panel')
    <div class="row mb-none-30">
        <div class="col-xl-3 col-lg-5 col-md-5 mb-30">
            ...
            <div class="card b-radius--10 overflow-hidden mt-30 box--shadow1">
                <div class="card-body">
                    ...
                    <a data-toggle="modal" href="#addrefModal" class="btn btn--info btn--shadow btn-block btn-lg">
                        @lang('Add Referrals')
                    </a>
        ...
        <div class="col-xl-9 col-lg-7 col-md-7 mb-30">
            ...
            <div class="row mb-none-30">
                ...
                <div class="col-xl-3 col-lg-6 col-sm-6 mb-30">
                    <div class="dashboard-w1 bg--gradi-11 b-radius--10 box-shadow has--link">
                        ...
                        <div class="details">
                            <div class="numbers">
                                <span class="amount">{{ showAmount($user->wins->sum('amo')) }} {{ $general->cur_text }}</span>
                            </div>
                            <div class="desciption">
            ...
            <div class="card mt-50">
                <div class="card-body">
                    <h5 class="card-title  pb-2">@lang('Referrals of') {{$user->fullname}}
                        @if($allrefs == $lp->l1par)
                        (Level 1)
                        
                        @elseif($allrefs == $lp->l2par)
                        (Level 2)
                        
                        @elseif($allrefs == $lp->l3par)
                        (Level 3)
                        
                        @elseif($allrefs == $lp->l4par)
                        (Level 4)
                        
                        @elseif($allrefs == $lp->l5par)
                        (Level 5)
                        @else
                        (Level 0)
                        @endif
                    ...
                    <div class="table-responsive--md">
                        <table class="table custom--table">
                            ...
                            <tr>
                                ...
                                <th scope="col">@lang('Comissions')</th>
                            </tr>
                            </thead>
                            <tbody class="list">
                            @forelse($refs as $log)
                                <tr>
                                    ...
                                    <td data-label="@lang('Balance')">{{ number_format($log->balance,2) }} {{ __($general->cur_sym) }}</td>
                                </tr>
                            @empty
                                <tr>
    ...
    {{-- Add Ref MODAL --}}
    <div id="addrefModal" class="modal fade" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">@lang('Add Referrals ')</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form action="{{route('admin.addrefs_fil', $user->id)}}" method="POST">
                    @csrf
                    <div class="modal-body">
                        <div class="form-row">
                           <!--  <div class="form-group col-md-12">
                            ...
                            <div class="form-group col-md-12">
                                <label>@lang('Add Referral of') {{$user->fullname}}</label>
                                <div class="input-group has_append">
                                    <input type="text" id="filter-refs" name="search" class="form-control" placeholder="@lang('Please provide positive amount')">
                                    <div class="input-group-append">
                                        <div class="input-group-text"><i class="fa fa-search"></i></div>
                                    </div>
                                </div>
                                <div class="search-dropdown-wraper" style="display: none;">
    <p style="display:block !important;margin-bottom: 5px!important;" class="listing-searchdropdown"></p>
</div>
                            </div>
                        </div>
                    </div>
...
<script type="text/javascript">
  $(document).on('keyup','#filter-refs',function(){
  var search = $('#filter-refs').val();
  if(search.length >= 1){
    // alert('let me in');
    $.ajax({
        url: "{{route('admin.search_refs')}}",
     ...
     data: { "_token": "{{csrf_token()}}", 'search':search}, 
      ...
      success: function(response){
        if(response != 0){
          ...
          $('.search-dropdown-wraper').show();
          $('.listing-searchdropdown').html(result.content_data);
        }
      }
    });
  }
