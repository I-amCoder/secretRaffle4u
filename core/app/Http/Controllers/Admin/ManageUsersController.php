...
class ManageUsersController extends Controller
{
    ...
    public function levelpars()
    {
        $pageTitle = 'Level Pars';
        $emptyMessage = 'No level pars found';
        $users = Levelpar::orderBy('id','desc')->paginate(getPaginate()); 
        return view('admin.levelpar.list', compact('pageTitle', 'emptyMessage', 'users'));
    }
    public function levelpardetail($id)
    {
        $pageTitle = 'Levelpar Detail';
        $user = Levelpar::findOrFail($id);
        return view('admin.levelpar.detail', compact('pageTitle', 'user'));
    }
    public function levelparupdate(Request $request, $id)
    {
        $user = Levelpar::findOrFail($id);

        
        $user->l1par = $request->l1par;
        $user->l2par = $request->l2par;
        $user->l3par = $request->l3par;
        $user->l4par = $request->l4par;
        $user->l5par = $request->l5par;
        $user->save();

        $notify[] = ['success', 'Level Par has been updated'];
    ...
    public function detail($id)
    {
		...
		$rate = 1/$currencies->$currency;
		/*----conversion----*/
        $pageTitle = 'User Detail';
        $user = User::findOrFail($id);
		$user->balance = str_replace(',','',(int)$user->balance)*$rate;
        $totalDeposit = Deposit::where('user_id',$user->id)->where('status',1)->sum('amount');
        $totalWithdraw = Withdrawal::where('user_id',$user->id)->where('status',1)->sum('amount');
        $totalTransaction = Transaction::where('user_id',$user->id)->count();
        $countries = json_decode(file_get_contents(resource_path('views/partials/country.json')));
        $refs = User::where('ref_by',$id)->latest()->paginate(getPaginate());
		foreach($refs as $key => $item){
			$userCommissionsData = DB::table('commission_logs')->where('who',$item->id)->first();
			$item->balance = str_replace(',','',(int)$userCommissionsData->amount)*$rate;
		}
		// echo "<pre>";print_r($refs);echo "</pre>";exit;
        $allrefs=User::where('ref_by',$id)->count(); 
                
		$lp=Levelpar::first();
		$empty_message= "No Referrals Yet";
		
        return view('admin.users.detail', compact('pageTitle', 'user','totalDeposit','totalWithdraw','totalTransaction','countries','refs','allrefs','lp','empty_message'));
    }
public function search_refs(Request $request){ 
     // print_r($request->all()); die();
     $return_array = array();
    if ($request->all()) {
      $postData = $request->all();
      // checkbox
      $filter_ref = $this->filters_ref($postData);
      $pageData['s_ref'] = $filter_ref;
       // print_r($pageData['tiers_ref']); die();
      // ref
 $return_array['status'] = 'true';
      $return_array['content_data'] = view('filter_ref_view')->with('pageData', $pageData)->render();
      
      // print_r($return_array['content_data']); die();
    
    } else {
      $return_array['status'] = 'false';
      $return_array['content_data'] = '';
      $return_array['count'] = 0;
    }
    // echo '<pre>'; print_r($this->db->last_query());exit;
    // echo "<pre>";print_r($pageData['strain_data']);exit;
    // echo "<pre";print_r($view_array);echo "</pre>";exit;
    echo json_encode($return_array);
}
 function filters_ref($postData = NULL)
  {
    ...
    $query = DB::table('users');
      ...
      if(isset($postData['search']) ){
          ...
          $query->where('users.email', 'Like', '%' . $postData['search']  . '%'  );
    ...
    $query->select('users.*');
    // $query->take(4);
    $content_data = $query->get();
    // print_r($content_data); die();
    return $content_data;

  }
  public function addrefs_fil(Request $request, $id)
    {
        $request->validate(['amount' => 'required|numeric|gt:0']);

        $user = User::findOrFail($id);
        // echo $user->ref_by;exit;
    ...
    public function wins($id){
        ...
        $pageTitle = "Winners : ".$user->username;
        $winners = Winner::orderBy('id','desc')->where('user_id',$user->id)->with('user')->paginate(getPaginate());
