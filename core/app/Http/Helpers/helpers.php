...
function loadCustomCaptcha($height = 46, $width = '100%', $bgcolor = '#003', $textcolor = '#b4903a')
{
...
function getFreeTicket($game_id, $purchased_ticket)
{
    return DB::table('raffle_game_free_lucky_draw')
        ->select('raffle_game_free_lucky_draw.free_ticket', 'raffle_game_free_tickets.name as ticket_name')
        ->where('raffle_game_free_lucky_draw.raffle_game_id', $game_id)
        ->where('raffle_game_free_lucky_draw.purchased_ticket', $purchased_ticket)
        ->leftJoin('raffle_game_free_tickets', 'raffle_game_free_lucky_draw.raffle_game_free_ticket_id', 'raffle_game_free_tickets.id')
        ->get();
}

function freetickets($game_id)
{
    $data = DB::table('raffle_game_free_ticket_map')->select('raffle_game_free_tickets.*')->whereIn('raffle_game_id', $game_id)->leftJoin('raffle_game_free_tickets', 'raffle_game_free_tickets.id', 'raffle_game_free_ticket_map.raffle_game_free_ticket_id')->get();
    return $data;
}



function getRafflePhoto($path)
{
    if ($path) {
        return asset('assets/images/game/' . $path);
    } else {
        return asset('assets/images/no_raffle.png');
    }
}


function getFreeTicketPhoto($path)
{
    if ($path) {
        return asset('assets/images/game/' . $path);
    } else {
        return asset('assets/images/no_free_ticket.png');
    }
}

function getCategoryPhoto($path)
{

    if ($path) {
        return asset('assets/images/category/' . $path);
    } else {
        return asset('assets/images/category/1.png');
    }
}
function getCurrSym()
{
